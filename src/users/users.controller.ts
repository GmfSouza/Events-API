...
export class UsersController {
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        name: { type: 'string', example: 'Gabryel' },
        email: {
          type: 'string',
          format: 'email',
          example: 'gabryel@example.com',
        },
        password: {
          type: 'string',
          format: 'password',
        ...
        profileImage: {
          ...
          format: 'binary',
          description:
            'Optional profile image file. Supported formats: jpg, jpeg, png, webp. Max size: 5MB',
          example: 'profile.jpg',
          nullable: true,
        },
      },
  ...
  async createUser(
    ...
    if (profileImageFile) {
      this.logger.log(
        `Image profile file received: ${profileImageFile.originalname}, size: ${profileImageFile.size} bytes`,
      );
    }
  ...
  async getUser(
    ...
    if (authUser.role !== UserRole.ADMIN && authUser.userId !== id) {
      this.logger.warn(
        `Unauthorized access attempt by user: ${authUser.userId} to get user: ${id}`,
      );
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Limit items per page (default 10, max 50)',
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Key to continue pagination (JSON stringified)',
  })
  @ApiResponse({
    status: 200,
    description: 'List of users and pagination information.',
    schema: {
      type: 'object',
      properties: {
        users: {
        ...
        lastEvaluatedKey: {
          type: 'object',
          example: { id: { S: 'some-id' } },
          nullable: true,
          description: 'Key for the next page, if it exists.',
        },
      },
    },
  })
  ...
  @ApiResponse({ status: 500, description: 'Internal server error.' })
  async getAll(
    @Query() listUserDto: ListUsersDto,
    @Req() request: AuthenticatedRequest,
  ): Promise<{
  ...
  @ApiParam({
    name: 'id',
    description: 'ID of the user to be updated (UUID)',
    type: String,
  })
  @ApiBody({
    type: UpdateUserDto,
  ...
  async updateUser(
    ...
    if (authUser.userId !== id) {
      this.logger.warn(
        `Unauthorized access attempt by user: ${authUser.userId} to update user: ${id}`,
      );
      throw new ForbiddenException(
        'You do not have permission to access this resource',
      );
  ...
  @ApiParam({
    name: 'id',
    description: 'ID of the user to be soft deleted (UUID)',
    type: String,
  })
  ...
  async delete(
    ...
    if (authUser.role !== UserRole.ADMIN && authUser.userId !== id) {
      this.logger.warn(
        `Unauthorized access attempt by user: ${authUser.userId} to delete user: ${id}`,
      );
      throw new ForbiddenException(
        'You do not have permission to access this resource',
      );
