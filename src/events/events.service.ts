import { PutCommand, QueryCommand, GetCommand, UpdateCommandInput, UpdateCommand } from '@aws-sdk/lib-dynamodb';
...
export class EventsService {
    ...
    async update(eventId: string, updateEventDto: UpdateEventDto, requesterId: string, eventImage?: Express.Multer.File): Promise<EventResponseDto> {
        this.logger.log(`Updating event with ID: ${eventId}, requesterId: ${requesterId}`);
        ...
        if (!(isAdmin || (isOrganizer && isOwner))) {
            this.logger.warn(`User with ID ${requesterId} is not authorized to update this event`);
            throw new ForbiddenException('You are not authorized to update this event');
        }

        this.logger.log(`User with ID ${requesterId} is authorized to update event: ${eventId}`);
        ...
        if (updateEventDto.description && updateEventDto.description !== event.description) {
        ...
        if (updateEventDto.organizerId && updateEventDto.organizerId !== event.organizerId) {
            if (!isAdmin) {
                throw new ForbiddenException('Only admins can change the organizer');
            }
            const newOrganizer = await this.usersService.findUserById(updateEventDto.organizerId);
            if (!newOrganizer) {
                this.logger.warn(`New organizer with ID ${updateEventDto.organizerId} not found`);
                throw new NotFoundException('New organizer not found');
            }

            if (newOrganizer.role !== UserRole.ORGANIZER && newOrganizer.role !== UserRole.ADMIN) {
                this.logger.warn(`User with ID ${newOrganizer.id} is not authorized to be an organizer`);
                throw new ForbiddenException('The new organizer doesn\'t have permission to be an organizer');
            }
        ...
        if (eventImage) {
            ...
            try {
                ...
                this.logger.warn(`Error parsing event image URL: ${event.imageUrl}`, error.stack);
            }
            ...
            try {
                ...
                this.logger.log(`New event image uploaded successfully: ${newImageUrl}`);
            } catch (error) {
                this.logger.error(`Error uploading event image for event: ${eventId}`, error.stack);
            }
        }

        if (!changes && updateExpressionParts.length === 1 && updateExpressionParts[0].startsWith('#updatedAt')) {
        ...
        try {
            const response = await this.dynamoDBService.docClient.send(new UpdateCommand(updateCommandinput));
            this.logger.log(`Event updated successfully: ${eventId}`);
            if (s3OldKeyToDelete && s3NewUploadKey && newImageUrl !== event.imageUrl) {
                this.logger.log(`Deleting old event image from S3: ${s3OldKeyToDelete}`);
                try {
                    await this.s3Service.deleteFile(s3OldKeyToDelete);
                    this.logger.log(`Old event image deleted successfully: ${s3OldKeyToDelete}`);
                } catch (error) {
                    this.logger.error(`Error deleting old event image from S3: ${s3OldKeyToDelete}`, error.stack);
                }
            }
            ...
            if (s3NewUploadKey && newImageUrl !== event.imageUrl) {
                this.logger.warn(`Error updating event: ${eventId}, removing new uploaded image from S3`, error.stack);
